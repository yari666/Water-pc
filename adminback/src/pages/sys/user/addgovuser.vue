...
<template>
  <div>
    <el-form ref="formRef" :model="form" :rules="formRules" label-width="100px">
      <el-tabs type="border-card">
        <el-tab-pane label="账号信息">
          ...
          <el-form-item label="监管级别" prop="regulatoryLevel">
            <el-select v-model="form.regulatoryLevel" placeholder="请选择">
              <el-option
        ...
        <el-tab-pane label="基础信息">
          ...
          <el-form-item label="负责人" prop="companyLegalPerson">
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="$emit('closegovdialog', false)">取 消</el-button>
...
export default {
  data() {
    return {
      ...
      formRules: {
        ...
        regulatoryLevel: [
          { required: true, message: "请选择监管级别", trigger: "blur" },
        ],
  ...
  methods: {
    ...
    // 监管级别下拉数据
    async getRegulatoryLevel() {
      const { data: result } = await getRegulatoryLevel(this.$http);

      if (result !== undefined) {
        this.regulatoryList = result;
      }
    },
    // 区域联动数据
    async getAreaList() {
    ...
    onSubmit() {
      ...
      this.$refs.formRef.validate(async (valid) => {
        ...
        if (this.openType != "edit") {
          const { data: result } = await createGovUser(this.$http, this.form);

          if (result !== undefined && result.id !== "") {
            this.$message.success("添加监管账号成功");

            this.$emit("closegovdialog", false);
            if (this.refreshList) {
              this.refreshList();
            }
          } else {
          ...
          const { data: result } = await updateGovUser(this.$http, this.form);
